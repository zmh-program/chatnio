...
function reducer(state: ChargeProps, action: any): ChargeProps {
    switch (action.type) {
        ...
        case 'add-model':
            ...
            if (model.length === 0 || state.models.includes(model))
                return state;
        ...
        <>
            ...
            <Dialog open={siteOpen} onOpenChange={setSiteOpen}>
                <DialogContent>
                    ...
                    <div
                        className={`pt-1 flex flex-row items-center justify-center`}
                    >
    ...
    return (
        <div className={`charge-editor`}>
            ...
            {form.type === nonBilling && (
                <div
                    className={`flex flex-row w-full h-max items-center mt-4 mb-6`}
                >
                    ...
                    {data.map((charge, idx) => (
                        <TableRow key={idx}>
                            ...
                            <TableCell>
                                {charge.input === 0
                                    ? 0
                                    : charge.input.toFixed(3)}
                            </TableCell>
                            <TableCell>
                                {charge.output === 0
                                    ? 0
                                    : charge.output.toFixed(3)}
                            </TableCell>
                            <TableCell>{t(String(charge.anonymous))}</TableCell>
                            <TableCell>
                                <div
                                    className={`inline-flex flex-row flex-wrap gap-2`}
                                >
                                    <OperationAction
