...
import {
  selectCurrent, selectGPT4,
  selectHistory,
  selectMessages, selectWeb,
  setGPT4,
  setWeb
} from "../store/chat.ts";
import {
                ...
                history.map((conversation, i) => (
                  <div className={`conversation ${current === conversation.id ? "active" : ""}`} key={i}
                    ...
                    <AlertDialog>
                      ...
                      <AlertDialogContent>
                        <AlertDialogHeader>
                          <AlertDialogTitle>{ t('conversation.remove-title') }</AlertDialogTitle>
                          <AlertDialogDescription>
                            { t('conversation.remove-description') }
                            <strong className={`conversation-name`}>{ conversation.name }</strong>
                            { t('end') }
                          </AlertDialogDescription>
                        </AlertDialogHeader>
                        <AlertDialogFooter>
        ...
        messages.map((message, i) => (
          <div className={`message ${message.role}`} key={i}>
            <div className={`message-content`}>
              <BingComponent keyword={message.keyword} />
              {
        ...
        <div className={`chat-wrapper`}>
          ...
          <div className={`chat-input`}>
            <div className={`input-wrapper`}>
              <TooltipProvider>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Toggle aria-label={ t('chat.web-aria') }
              ...
              <Input id={`input`} className={`input-box`} ref={target} placeholder={ t('chat.placeholder') } onKeyDown={
                async (e: React.KeyboardEvent<HTMLInputElement>) => {
                  if (e.key === "Enter") await handleSend(auth, gpt4, web);
                }
              } />
              <Button size={`icon`} variant="outline" className={`send-button`} onClick={
                () => handleSend(auth, gpt4, web)
              }>
